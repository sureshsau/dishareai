...
const UsedTechSales = () => {
  ...
  const benefits = [
    { icon: <FaUndo className="text-[#ffd700] text-3xl" />, title: "Easy Returns", desc: "Hassle-free returns & exchanges." },
    { icon: <FaShieldAlt className="text-[#ffd700] text-3xl" />, title: "1-Year Warranty", desc: "Enjoy peace of mind with our warranty." },
    { icon: <FaTag className="text-[#ffd700] text-3xl" />, title: "Best Price", desc: "Affordable pricing on all devices." },
    { icon: <FaCheckCircle className="text-[#ffd700] text-3xl" />, title: "Quality Checked", desc: "Each phone is tested & certified." },
  ];
  ...
  return (
    <section className="py-20 bg-gray-100">
      <div className="max-w-7xl mx-auto px-4 text-center">
        ...
        {/* 📢 Campaign Slogan */}
        <h2 className="text-5xl font-extrabold text-[#1a237e] mb-8 uppercase tracking-wide">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-8">
            {mobileProducts.map((image, index) => (
              <div key={index} className="bg-white rounded-xl shadow-lg p-4 flex justify-center items-center hover:shadow-2xl transition-shadow">
                <img
                  src={image}
                  alt={`Phone ${index + 1}`}
                  className="w-full h-48 object-cover rounded-md border border-gray-200 transform hover:scale-105 transition-transform"
                />
              </div>
            ))}
          </div>
